...
<div class="wrap">
			...
			<ul>
				...
				<div class="user">
					...
					echo empty($session['user']) ? (
						...
						'<a class="padding-- animation_header_a" id="signin" href="/web/site/login">用户名</a>'.
						'<a class="padding-- animation_header_a" id="register" href="/web/site/logout">注销</a>'
					);
					?>
				</div>
			</ul>
	...
	<div class="bigBackground" style="display:{{:signinForm}}" id="signinForm" onclick="closeWin()">
		<div class="siginForm_div">
			...
			<form id="userSigninForm" class="userForm" name="userSigninForm" method="post" action="/web/user/signin">
				<img class="logo" src="/views/public/images/logo64x64.png" />
				<input type='hidden' name='_csrf' value="{{:csrf}}"/>
				<input class="login_input" name="UserSigninForm[phone]" autoComplete="off" placeholder="手机号码" type="text" />
				<input class="login_input" name="UserSigninForm[password]" placeholder="密码" type="password" />
	...
	<div class="bigBackground" style="display:{{:registerForm}}" id="registerForm" onclick="closeWin()">
		<div class="registerForm_div">
			<!--<div class="close_btn" onclick="closeWin('signinForm')">×</div>-->
			<form id="userRegisterForm" class="userForm" name="userRegisterForm" method="post" action="{{:ip}}web/user/signin">
